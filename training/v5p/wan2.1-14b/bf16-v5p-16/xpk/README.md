# Pretrain wan workload on v5p GKE clusters with XPK
...
-   `WORKLOAD_NAME`: A unique name for your workload. This is set in
    `run_recipe.sh` to `$(printf "%.26s" "${USER//_/-}-wan")-$(date +%Y%m%d-%H%M)`
    by default.
...
cd tpu-recipes/training/v5p/wan2.1-14b/bf16-v5p-16/xpk
```
