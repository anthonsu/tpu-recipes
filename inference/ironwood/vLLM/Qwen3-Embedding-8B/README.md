# Serve Qwen3-Embedding-8B with vLLM on TPU Ironwood

In this guide, we show how to serve **Qwen3-Embedding-8B** model using vLLM on Ironwood (TPU7x).
...
4. Save this yaml file as `vllm-tpu.yaml`
    ...
    **Note:** This config uses 16k input. If users want to run
    ...
    spec:
      ...
      template:
        ...
        spec:
          ...
          containers:
          - name: vllm-tpu
            image: vllm/vllm-tpu:nightly-20260730-30dcf7a-48aa8d8
...
To benchmark the server, we use the vLLM benchmark client.
